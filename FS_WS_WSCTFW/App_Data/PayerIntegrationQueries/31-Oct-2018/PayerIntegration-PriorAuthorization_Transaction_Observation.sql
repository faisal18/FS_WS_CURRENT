...
'				(CASE when Prov.Region = 2 then Convert(decimal(20,2),(PAA.drug_quantity * (select DCR.granual_units from PBMM..DRUG_CODE_REF DCR with (nolock) where DCR.id = PAA.drug_id))) when Prov.Region != 2 then Convert(decimal(20,2),(PAA.drug_quantity)) END) as ''Quantity'','+
