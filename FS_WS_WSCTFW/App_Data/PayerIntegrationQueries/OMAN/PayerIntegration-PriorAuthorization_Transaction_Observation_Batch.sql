...
where 
 at.payer_id = @Payerid
and PA.download_batch_id in ('26396df4-6107-44d5-8925-a55d39b07d63','48b1ebe7-9d87-48e9-a236-8b5f68489a57')
...
' INNER JOIN PBMM..PRIOR_AUTHORIZATION PA2 with (nolock) on PA2.id = at.id'+
' INNER JOIN PBMM..BATCH_DOWNLOAD_TRANS BDT with (nolock) on PA2.download_batch_id = BDT.batch_id'+
'  where at.id = ' + cast(@TransactionID as varchar(1000)) +
'  FOR XML PATH(''Authorization''), type '-- ROOT('''')'

declare @filename varchar(2000)
set @filename = ''
set @filename = 'C:\TMP\omar\PA\PA_' + cast(@transactionID as varchar(1000))+ '.XML'
