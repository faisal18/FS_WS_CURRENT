...
-- where 
 -- at.payer_id = @Payerid
-- --and 
-- --PR.download_canceled_batch_id in (null, '0')
-- and 
-- at.state_id = 8
...
id from PRIOR_REQUEST PR with (nolock)
inner join BATCH_DOWNLOAD_TRANS BD with (nolock) on PR.download_canceled_batch_id = bd.batch_id
where 
bd.is_downloaded = 0
and
...
' PRA.type as ''Type'', '+
' PRA.code as ''Code'','+
' Convert(decimal(9,2),PRA.quantity) as ''Quantity'','+
...
' inner join PBMM..BATCH_DOWNLOAD_TRANS BDT with (NoLock) on BDT.batch_id = PR2.download_canceled_batch_id '+
...
set @filename = ''
set @filename = 'c:\tmp\PR_Cancel\BCR_' + cast(@transactionID as varchar(1000))+ '.XML'
