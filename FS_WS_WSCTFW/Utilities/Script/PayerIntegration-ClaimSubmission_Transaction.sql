...
at.payer_id in (select PAY2.id from PBMM..PAYER PAY2 with(nolock) where bucket_mapping_id in (1,3,4,5,7,54,1247,1285,1325,1333,1350,1364,1275))
...
'					(CASE'+
'                           WHEN Prov.Region = 2 '+
'						   THEN Convert(decimal(10,2),(select PRA.pbm_quantity from PBMM..PRIOR_REQUEST_ACTIVITY PRA with(nolock) where PRA.code = CSA.code and PRA.drug_id = CSA.drug_id and PRA.prior_request_id = CSA.submission_id))'+
'                           WHEN Prov.Region = 3 AND PAY2.bucket_mapping_id = 7 '+
'						   THEN Convert(decimal(10,2),(select PRA.pbm_quantity from PBMM..PRIOR_REQUEST_ACTIVITY PRA with(nolock) where PRA.code = CSA.code and PRA.drug_id = CSA.drug_id and PRA.prior_request_id = CSA.submission_id))'+
'                           ELSE Convert(decimal(10,2),(CSA.quantity))'+
'                       END) AS ''Quantity'','+
